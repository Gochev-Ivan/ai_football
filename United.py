...
def strategija2(our_team, their_team, ball, your_side, half, time_left, our_score,
...
def zona(our_side, x, y):
    ...
    if strategija == 1:
        ...
        if (x < 0.2 * duzina and x > 0.08 * duzina) or (x < 0.08 * duzina and y > post_gore and y < post_dole):
            return 0
        elif x > 0.25 * duzina:
            return 2
        else:
...
def init(our_team, ball):
    ...
    agolman = our_team[0]['alpha']
    adribler = our_team[1]['alpha']
    abloker = our_team[2]['alpha']
...
def po_y_osi(golman, y_u_golu, ogranicenje):
    ...
    golman['force'] = sila
    golman['alpha'] = ugao
    golman['shot_request'] = True
    golman['shot_power'] = 1111111110
...
def min_vreme_do_lopte(player, ball):
    ...
    if (vlopte * np.sin(beta) - player['v_max'] * np.sin(alfa1)) == 0:
        im1 = 0.01
    else:
        im1 = vlopte * np.sin(beta) - player['v_max'] * np.sin(alfa1)
    ts1 = (yi - yl) / im1
    if (vlopte * np.sin(beta) - player['v_max'] * np.sin(alfa2)) == 0:
        im2 = 0.01
    else:
        im2 = vlopte * np.sin(beta) - player['v_max'] * np.sin(alfa2)
    ts2 = (yi - yl) / im2
    if ts1 * ts2 > 0:
        return 1000000
    elif ts1 > 0:
...
def trci(player, x, y):
    if (x - player['x'] == 0 and y - player['y'] == 0):
        player['alpha'] = 0
    else:
...
def dzoni_prilazak_lopti(player, ball, xz, yz):
    player['shot_request'] = True
    player['shot_power'] = 1000000000
    xi = player['x']
    yi = player['y']
    rl = ball['radius']
    ri = player['radius']

    if provera_da_li_moze_direktno(player, ball, xz, yz):

        trci(player, xz, yz)
    else:

        xg, yg, xd, yd = tangenta(xi, yi, ball['x'], ball['y'], ri + rl + 1)
        if rastojanje(xi, yi, ball['x'], ball['y']) > rl + ri + 3:

            trci(player, ball['x'], ball['y'])
        else:

            if rastojanje(xz, yz, xd, yd) < rastojanje(xz, yz, xg, yg):
...
def gde_se_pozicionirati_za_sut(dribler, ball, our_side, njihov_golman):
    ...
    if (njihov_golman['x'] - njihov_golman['radius'] >= xgola and our_side == 'left') or (
        ...
        if im == 0:
            ...
            xod = ((yod - ylopte) * xgola + xlopte * (yod - yst)) / im
    else:
        xod = xgola
        if njihov_golman['y'] - ytop < ybot - njihov_golman['y']:
            yod = post_screen_bottom - post_radius - ball['radius'] - 10
        else:
            yod = post_screen_top + post_radius + ball['radius'] + 10
...
def igraci_zhmk(our_team, their_team, ball, your_side, half, time_left, our_score, their_score):
    ...
    if ((our_score > their_score+1) or (our_score+1<their_score)):
        return strategija2(our_team, their_team, ball, your_side, half, time_left, our_score, their_score)
    else:
...
def strategija1(our_team, their_team, ball, your_side, half, time_left, our_score,
    ...
    bloker_x_zeljeno = poslednji_igrac['x']
    if bloker['x'] * nasa_strana > (post_screen_left + post_screen_right) * 1.1 * nasa_strana / 2:
        ...
        bloker_y_zeljeno = post_screen_bottom
    trci(bloker, bloker_x_zeljeno, bloker_y_zeljeno)
    bloker['shot_request'] = False
    bloker['shot_power'] = 0
    ...
    if dr_pozicionirao_se == 1:
        ...
        #         complex(dribler['x'] - ball['x'], dribler['y'] - ball['y']))) <= 0.15 and u_kontaktu(dribler, ball):
        #     sutni(dribler, ball)
        if u_kontaktu(dribler, ball):
            sutni(dribler, ball)
            dr_pozicionirao_se = 1
        else:
            dzoni_prilazak_lopti(dribler, ball, xzelj, yzelj)
    else:
        if np.cos(ball['alpha'] * nasa_strana) > 0:
            # if nasa_strana == 1:
            #     njihov_gol = post_screen_right
            ...
            if ball['x'] * nasa_strana > dribler['x'] * nasa_strana:
                ...
                dzoni_prilazak_lopti(dribler, ball,
                                     ball['x'] + (ball['radius'] + dribler['radius']) * np.cos(ball['alpha']),
                                     ball['y'] + (ball['radius'] + dribler['radius']) * np.sin(ball['alpha']))
                dribler['shot_request'] = False
                dribler['shot_power'] = 0
...
# def strategija2(our_team, their_team, ball, your_side, half, time_left, our_score,
#                 their_score):  # bloker na golmanu, centralni kao defanzivni vezni koji ispucava lopte:
#     global dr_force
#     global dr_alpha
#     global dr_pozicionirao_se
#     global dr_radnja_def
#     ytop = 50 + int(resolution[1] / 5)
#     ybot = resolution[1] - 50
#     bloker = our_team[2]
#     dribler = our_team[1]
...
#     bloker['shot_power'] = 0
#
#     # defanzivac stanja: 0 - idi na svoje mesto (0.33); 1 - prati po y; 2 - istrci i ispucaj; 3 - skloni se
#
#     if your_side == 'left':
#         nasa_strana = 1
#     else:
#         nasa_strana = -1
#
#     if not na_golu_st2(your_side, dribler, ball) and (nasa_strana * pravac * vlopte) >= 0:  ######################3
#         dr_radnja_def = 0
#     elif ball['x'] * nasa_strana < dribler['x'] * nasa_strana:
#         dr_radnja_def = 3
#     else:
#         if zona_st2_def(your_side, ball['x'], ball['y']) == 0:
#             dr_radnja_def = 2
#         else:
#             dr_radnja_def = 1
#
#     if dr_radnja_def == 0:
#         vrati_se_na_def(dribler, ball, your_side)
#     if dr_radnja_def == 1:
#         po_y_osi_def(dribler, ball['y'])
#     if dr_radnja_def == 2:
#         istrci_i_ispucaj_def(ball, dribler, poslednji_igrac, your_side)
#     if dr_radnja_def == 3:
#         if your_side == 'left':
#             xzelj = post_screen_left + (post_screen_right - post_screen_left) * 0.33 + post_radius + ball[
#                                                                                                          'radius'] * 0.8 + \
#                     dribler['radius']
#         else:
#             xzelj = post_screen_right - (post_screen_right - post_screen_left) * 0.33 - post_radius - ball[
#                                                                                                           'radius'] * 0.8 - \
#                     dribler['radius']
#         if (dribler['y'] - ytop < ybot - dribler['y']):
#             yzelj = ytop
#         else:
#             yzelj = ybot
#         trci(dribler, yzelj, xzelj)
#         dribler['shot_request'] = False
#         dribler['shot_power'] = 0
#         dribler['force'] = 100009000
#     return dribler['alpha'], dribler['force'], dribler['shot_request'], dribler['shot_power'], bloker['alpha'], bloker[
#         'force'], bloker['shot_request'], bloker['shot_power']


def u_kontaktu(player, ball):
    return (rastojanje(player['x'], player['y'], ball['x'], ball['y']) <= player['radius'] + ball['radius'] + 1)


def na_golu_st2(our_side, player, ball):
    if our_side == 'left':
        return player['x'] <= (post_screen_right - post_screen_left) * 0.33 + post_radius + ball['radius'] * 0.8 + \
                              player['radius']
    else:
        return player['x'] >= (post_screen_right - post_screen_left) * 0.67 - post_radius - ball['radius'] * 0.8 - \
                              player['radius']


def zona_st2(our_side, x, y):
    ytop = 50 + int(resolution[1] / 5)
    duzina = resolution[0] - 100
    x -= 50
    y -= ytop
    ...
    if (x < 0.13 * duzina and x > 0.08 * duzina) or (x < 0.08 * duzina and y > post_gore and y < post_dole):
        return 0
    elif x > 0.25 * duzina:
        return 2
    else:
...
def zona_st2_def(our_side, x, y):
    ...
    if (x < 0.35 * duzina and x > 0.2 * duzina):
        return 0
    else:
        return 1  # Kada je lopta desno, pozicioniraj se po y, kada je levo, bezi u stranu


def vrati_se_na_def(player, ball, our_side):
    ytop = 50 + int(resolution[1] / 5)
    ybot = resolution[1] - 50
    if player['y'] < ytop:
        y = ytop
        ...
        y = player['y']
    if our_side == 'left':
        x = (post_screen_right - post_screen_left) * 0.2 + post_screen_left + post_radius + ball['radius'] * 0.8 + \
            player['radius']
    else:
        x = post_screen_right - (post_screen_right - post_screen_left) * 0.2 - post_radius - ball['radius'] * 0.8 - \
            player['radius']
    trci(player, x, y)
    player['shot_request'] = False
    player['shot_power'] = 0
...
def po_y_osi_def(golman, y_u_golu):
    global dr_prom_sm
    global dr_proslo_y
    ...
    if y_u_golu > ybot - golman['radius'] - 1:
        y_u_golu = ybot
    elif y_u_golu <= ytop + golman['radius'] + 1:
        y_u_golu = ytop
    ...
    if ugao != golman['alpha']:
        dr_prom_sm = dr_prom_sm + 1

    if dr_proslo_y != y_u_golu:
        dr_prom_sm = 0
        dr_proslo_y = y_u_golu
...
def istrci_i_ispucaj_def(ball, player, njihov_golman, our_side):
    if our_side == 'left':
        nasa_strana1 = 1
    else:
        nasa_strana1 = -1

        # if rastojanje(ball['x'], ball['y'], player['x'], player['y']) > player['radius'] + ball['radius'] + 2:
        #     xzeljeno, yzeljeno = gde_se_pozicionirati_za_sut(player, ball, our_side, njihov_golman)
        #     # pozicioniraj(ball, player, xzeljeno, yzeljeno, nasa_strana1)
        #     dzoni_prilazak_lopti(player, ball, xzeljeno, yzeljeno)
        # else:
        player['shot_power'] = 100000
        player['shot_request'] = True
        trci(player, ball['x'], ball['y'])
...
def decision(our_team, their_team, ball, your_side, half, time_left, our_score, their_score):
    ...
    manager_decision[2]['shot_power'] = igraci_zhmk(our_team, their_team, ball, your_side, half, time_left, our_score,
                                                    their_score)
    if np.isnan(manager_decision[0]['alpha']):
        manager_decision[0]['alpha'] = agolman

    if np.isnan(manager_decision[1]['alpha']):
        manager_decision[1]['alpha'] = adribler

    if np.isnan(manager_decision[2]['alpha']):
        manager_decision[2]['alpha'] = abloker
    ...
    if time_left <= 5:
        if our_team[0]['x'] * nasa_strana < ball['x'] * nasa_strana:
            manager_decision[0]['shot_power'] = 10000000
            manager_decision[0]['shot_request'] = True

        if our_team[1]['x'] * nasa_strana < ball['x'] * nasa_strana:
            manager_decision[1]['shot_power'] = 10000000
            manager_decision[1]['shot_request'] = True

        if our_team[2]['x'] * nasa_strana < ball['x'] * nasa_strana:
            manager_decision[2]['shot_power'] = 10000000
            manager_decision[2]['shot_request'] = True
